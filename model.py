...
class QTrainer:
    ...
    def train_step(self, state, action, reward, next_state, game_over_state):
        ...
        if len(state.shape) == 1:
            # (1, x) append one dimension in the beginning
            state = torch.unsqueeze(state, 0)
            next_state = torch.unsqueeze(next_state, 0)
            action = torch.unsqueeze(action, 0)
            reward = torch.unsqueeze(reward, 0)
            game_over_state = (game_over_state, )     # convert game over state to a tuple
        ...
        # 2: reward + gamma * max(next predicted Q value) -> only do this if game is not over
        # pred.clone()
